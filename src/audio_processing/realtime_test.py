            ...
            if volume > VOLUME_THRESHOLD:
                try:
                   ...
                   # ...existing code...
                    ...
                    man_scream_prob = prob[0]      # man_scream olasılığı
                    woman_scream_prob = prob[1]    # woman_scream olasılığı
                    noise_prob = prob[2]           # noise olasılığı
                    
                    print(f"Tespit: {predicted_class} (güven: {confidence:.3f})")
                    print(f"Ses seviyesi: {volume:.4f}")
                    print(f"Olasılıklar - Erkek: {man_scream_prob:.2f}, Kadın: {woman_scream_prob:.2f}, Gürültü: {noise_prob:.2f}")
                    
                    # ACİL DURUM KONTROLÜ - Kadın Çığlığı
                    # Koşullar:
                    # 1. Kadın çığlığı olarak algılandı
                    # 2. Güven %40'tan fazla
                    # 3. Kadın olasılığı erkekten %20 fazla
                    # 4. Gürültü olasılığı %30'dan düşük (müzik/arka plan değil)
                    if (predicted_class == 'woman_scream' and 
                        confidence > 0.4 and 
                        woman_scream_prob > man_scream_prob * 1.2 and
                        noise_prob < 0.3):
                    ...
                    # ERKEK ÇIĞLIĞI KONTROLÜ
                    # Koşullar:
                    # 1. Erkek çığlığı olarak algılandı
                    # 2. Güven %60'tan fazla (daha yüksek eşik)
                    # 3. Erkek olasılığı kadından %20 fazla
                    # 4. Gürültü olasılığı %30'dan düşük (müzik/arka plan değil)
                    elif (predicted_class == 'man_scream' and 
                          confidence > 0.6 and
                          man_scream_prob > woman_scream_prob * 1.2 and
                          noise_prob < 0.3):
                        print("\n⚠️ ERKEK ÇIĞLIĞI TESPİT EDİLDİ!")
                        print("⚠️ DİKKAT GEREKTİRİYOR!\n")
