...
class DlibFaceDetector(BaseFaceDetector):
    ...
    def __init__(
        ...
        Args:
            use_cnn: Use CNN-based detector (more accurate but slower)
            upsample_num_times: Upsample image before detection (more faces but slower)
            cnn_model_path: Path to CNN model file (required if use_cnn=True)
        """
        ...
        try:
            ...
            if use_cnn:
                if cnn_model_path is None:
                    ...
                    default_path = Path(__file__).parent.parent / "data" / "models" / "face_detection" / "mmod_human_face_detector.dat"
            ...
            predictor_path = Path(__file__).parent.parent / "data" / "models" / "face_detection" / "shape_predictor_68_face_landmarks.dat"
    ...
    def detect(self, image: np.ndarray) -> List[DetectedFace]:
        ...
        if self.use_cnn:
            ...
            # CNN returns mmod_rectangles
            faces_data = [
                (d.rect, d.confidence) for d in detections
            ]
        else:
            # HOG returns rectangles with scores
